...
class Student_home_pageTest5 {
    ...
    Student_home_pageTest5() throws SQLException {
    }
    @BeforeEach
    void setUp() {
        try{
            /*stmt.executeUpdate("insert into program_core values('CS203','Computer Science and Engineering',3,2020);");*/
            stmt.executeUpdate("update course_catalog set prerequisites='{\"CS210\",\"GE103\"}' where courseid='CS201';");
    ...
    void tearDown() {
        try{
            stmt.executeUpdate("update course_catalog set prerequisites='{\"GE103\"}' where courseid='CS201';");
